...
int FastHadronReco::process_event(PHCompositeNode* topNode)
{
...
std::vector<unsigned int> FastHadronReco::getTrackStates(SvtxTrack *track)
{
...
void FastHadronReco::match_calos(SvtxTrack* track1, SvtxTrack* track2, float& track1_cemc_phi, float& track1_cemc_eta, float& cemc_phi1, float& cemc_eta1, float& cemc_z1, float& cemc_ecore1, float& track2_cemc_phi, float& track2_cemc_eta, float& cemc_phi2, float& cemc_eta2, float& cemc_z2, float& cemc_ecore2, float& track1_ihcal_phi, float& track1_ihcal_eta, float& ihcal_phi1, float& ihcal_eta1, float& ihcal_e3x3_1, float& track2_ihcal_phi, float& track2_ihcal_eta, float& ihcal_phi2, float& ihcal_eta2, float& ihcal_e3x3_2, float& track1_ohcal_phi, float& track1_ohcal_eta, float& ohcal_phi1, float& ohcal_eta1, float& ohcal_e3x3_1, float& track2_ohcal_phi, float& track2_ohcal_eta, float& ohcal_phi2, float& ohcal_eta2, float& ohcal_e3x3_2, double Zvtx)
{
...
RawCluster* FastHadronReco::MatchClusterCEMC(SvtxTrackState* trackstate, RawClusterContainer* cemc_clusters, float& track_eta, float& track_phi, double Zvtx)
{
...
double FastHadronReco::Get_CAL_e3x3(SvtxTrackState* trackstate, TowerInfoContainerv4* _towersInfo, float& track_eta, float& track_phi, float& hcal_eta, float& hcal_phi, double Zvtx, int what)
{
...
void FastHadronReco::fillHistogram(Eigen::Vector3d mom1, Eigen::Vector3d mom2, float& decaymassA, float& decaymassB, TH1* massreco, double& invariantMass, double& invariantPt, float& invariantPhi, float& rapidity, float& pseudorapidity)
{
...
bool FastHadronReco::projectTrackToPoint(SvtxTrack* track, Eigen::Vector3d PCA, Eigen::Vector3d& pos, Eigen::Vector3d& mom)
{
  bool ret = true;
...
bool FastHadronReco::projectTrackToCylinder(SvtxTrack* track, double Radius, Eigen::Vector3d& pos, Eigen::Vector3d& mom)
{
...
Acts::Vector3 FastHadronReco::getVertex(SvtxTrack* track)
{
...
void FastHadronReco::findPcaTwoTracks(float& decaymassA, float& decaymassB, const Acts::Vector3& pos1, const Acts::Vector3& pos2, Acts::Vector3 mom1, Acts::Vector3 mom2, Acts::Vector3& pca1, Acts::Vector3& pca2, double& dca) const
{
  TLorentzVector v1;
  TLorentzVector v2;
...
FastHadronReco::FastHadronReco(const std::string& name)
  : SubsysReco(name)
{
}

Acts::Vector3 FastHadronReco::calculateDca(SvtxTrack* track, const Acts::Vector3& momentum, Acts::Vector3 position)
{
...
double FastHadronReco::get_dEdx_fitValue(float& qmomentum, int& PID)
{
...
int FastHadronReco::End(PHCompositeNode* /**topNode*/)
{
  fout->cd();
  ntp_reco_info->Write();
  recomass->Write();
...
int FastHadronReco::getNodes(PHCompositeNode* topNode)
{
